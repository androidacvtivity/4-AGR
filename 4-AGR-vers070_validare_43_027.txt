...
function agr4GetValue(values, fieldName, index) {
...
function agr4IsPositive(value) {
    return agr4ToNumber(value) > 0;
}

function agr4HasAnyCompletedCap1Row(values, filialSuffix, index) {
    var pattern = filialSuffix === '_FILIAL'
        ? /^CAP1_R(\d+)_C1_FILIAL$/
        : /^CAP1_R(\d+)_C1$/;
    ...
    for (var fieldName in values) {
        ...
        var match = fieldName.match(pattern);

        if (!match) {
            continue;
        }

        // R160 este rândul care se cere obligatoriu, nu se ia ca rând completat.
        if (match[1] === '160') {
            continue;
        }

        if (agr4IsPositive(agr4GetValue(values, fieldName, index))) {
            return true;
        }
    }
...
webform.validators.agr4 = function (v, allowOverpass) {
    ...
    // 43-027: Daca este cel putin un rand completat in capitolul static,
    // atunci randul 160 trebuie sa fie completat si mai mare ca 0.
    var hasCompletedStaticCap1Row = agr4HasAnyCompletedCap1Row(values, '', null);
    var r160Static = agr4ToNumber(values.CAP1_R160_C1);

    if (hasCompletedStaticCap1Row && r160Static <= 0) {
        webform.errors.push({
            'fieldName': 'CAP1_R160_C1',
            'weight': 27,
            'msg': Drupal.t('Cod eroare: 43-027 (Cap.1). Daca este completat cel putin un rand din Cap.1, atunci Rind.160 trebuie sa fie > 0 [@r160 <= 0]', { '@r160': r160Static })
        });
    }
    ...
    for (var j = 0; j < values.CAP_NUM_FILIAL.length; j++) {
        ...
        // 43-027: Daca este cel putin un rand completat in capitolul dinamic/filiala,
        // atunci randul 160 pentru aceeasi filiala trebuie sa fie completat si mai mare ca 0.
        var hasCompletedFilialCap1Row = agr4HasAnyCompletedCap1Row(values, '_FILIAL', j);
        var r160Filial = agr4ToNumber(agr4GetValue(values, 'CAP1_R160_C1_FILIAL', j));

        if (hasCompletedFilialCap1Row && r160Filial <= 0) {
            webform.errors.push({
                'fieldName': 'CAP1_R160_C1_FILIAL',
                'weight': 27,
                'index': j,
                'msg': Drupal.t('Cod eroare: 43-027 (Cap.1). Daca este completat cel putin un rand din Cap.1 pe filiala, atunci Rind.160 trebuie sa fie > 0 [@r160 <= 0]', { '@r160': r160Filial })
            });
        }
